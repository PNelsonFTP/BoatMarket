...
- **Service templates are portable; scheduler installation is machine-specific.** Windows CI is configured to exercise application paths and generated templates; its first run for these changes follows the owner's authorized final push; consult VALIDATION.md for the recorded result. It does not install a persistent user task on the owner's Windows computer. Laptop sleep, account logout and network failures still affect collection.
