...
| Public deployment and first-party license | September 10: owner authorized public repository visibility; reviewed Pages deployment succeeded with HTTPS enforced and public desktop/mobile verification passed. September 8's private-repository HTTP 422 is historical. `UNLICENSED` and npm `private: true` remain unchanged | An open-source licensing change remains a separate owner choice; future data updates require local collection/export, review, commit and manual deployment |
...
- **Make/category inference still needs review.** September 10 fixed `Tiger`/`TIGER` falsely matching `Tige` and explicit PWC labels losing to generic ski terms; eight supported current-capture repairs were applied. A 2027 Vexus ADX200 title mentioning a Yamaha engine still infers Yamaha as the boat make. Ambiguous PWC/accessory and trailer cases were intentionally left for review. Future corrections need source evidence and identity-preserving regression coverage, not broad speculative relabeling.
