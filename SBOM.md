...
- The two linux/arm64 image tests ran locally. The [first hosted run for `fa8a04c`](https://github.com/PNelsonFTP/BoatMarket/actions/runs/34260064907) passed both AMD64 image builds, smoke checks and full-schema runtime inventories: 3,956 default-runtime and 5,269 rendered components. Exact image IDs, scanner/output hashes and smoke reports are retained in its `runtime-sbom-runtime` and `runtime-sbom-rendered` artifacts and under ignored `data/p1p2/github-34260064907`. Application test failures from that run are documented separately in VALIDATION.md; container success does not imply application-suite success. The final compatibility changes add no dependencies and preserve the lock/source npm inventories above.
